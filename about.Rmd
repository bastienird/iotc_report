...
Les données georef sup nominales ne sont pas expliquées par des aggregations pour ces même strates (pour environ 10 000 tonnes)
...
```{r echo=TRUE}
lvl0_strata$`species, year, gear_type, fishing_fleet`$sum_georef_no_nominal +lvl0_strata$`species, year, gear_type, fishing_fleet`$sum_georef_sup_nom # 5 098 630
...
```{r fig.cap="Georef_sup_to_nom strate espèces/engins/années/fishing_fleet (5premières lignes)"}

datatable(head(lvl0_strata$`species, year, gear_type, fishing_fleet`$georef_sup_nominal),)
```


```{r fig.cap="Georef_not_nom strate espèces/engins/années/fishing_fleet (5premières lignes)"}

datatable(lvl0_strata$`species, year, gear_type, fishing_fleet`$georef_no_nominal_with_value)
...
On voit que pour la majorité des années, lisser les différences nom/geo en regroupant les engins permet de réduire ces différences. On peut donc supposer que ces problèmes proviennent de reports entre des engins pour une même strate. Le problème pourrait donc venir d'un mapping différent d'engins entre les nominal et les géoreferencés.
...
## Fonction de conversion des données IOTC de nombre à tonnes

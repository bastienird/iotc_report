...
Ce sont surtout en tonnes des données pour SKJ et YFT, et notamment pour 09.2 (lignes mécanisées et lignes à cannes mécaniques). Elles représenent un total de `r sum_georef_no_nominal_st_ff` tonnes soit `r (sum_georef_no_nominal_st_ff *100 ) /lvl0 %>% dplyr::filter(measurement_unit == "t") %>% dplyr::summarise(sum = sum(measurement_value)) %>% dplyr::pull()`% des données totales.
...
Elles représentent un total de `r round(sum_no_sup_st_ff_gr,3)` tonnes soit `r round((sum_no_sup_st_ff_gr *100 ) /lvl0 %>% dplyr::filter(measurement_unit == "t") %>% dplyr::summarise(sum = sum(measurement_value)) %>% dplyr::pull())`% des données totales.
...
datatable2(head(lvl0_strata$`species, year, gear_type, fishing_fleet`$georef_sup_nominal))
...
CA_RAISED_FILTERED_NO_FLEET <- read_csv("_site/data/CA_RAISED_FILTERED_NO_FLEET.csv", guess_max = 0, 
...
On calcule les facteurs de conversion moyen qu'il devrait y avoir pour que les données géoreferencées en nombre soit converties et atteignent les données nominales. Il y a un biais qui est que les données nominales doivent être supérieures aux données géoreferencées et donc on fait une conversion en même temps qu'un raise.

```{r include=FALSE}

source("~/firms-gta/geoflow-tunaatlas/tunaatlas_scripts/generation/convert_number_to_nominal.R") #modified for geoflow
# source("https://raw.githubusercontent.com/firms-gta/geoflow-tunaatlas/tunaatlas_script/convert_number_to_nominal.R")

lvl0_conv_only_double_unit <-lvl0 %>% 
          dplyr::mutate(time_start = as.character(time_start))%>% 
                                                             dplyr::mutate(time_end = as.character(time_end)) %>% 
          dplyr::group_by(time_start,time_end, species, fishing_fleet, gear_type, source_authority, fishing_mode) %>%
            dplyr::mutate(numberunit = n_distinct(measurement_unit)) %>% dplyr::filter(numberunit != 1)

try(convert_number_to_nominal_output_t_and_no <- convert_number_to_nominal(lvl0_conv_only_double_unit %>% dplyr::select(-c(measurement_type, measurement, measurement_status)), nominal%>% dplyr::select(-c(measurement_type, measurement, measurement_status)), raise_only_unmatched = FALSE, strata = c("species", "gear_type", "fishing_fleet", "year")))

raisingfactorfromnumber_to_nominal_t_and_no <- readRDS(here::here("data/raisingfactorfromnumber_to_nominal.rds")) %>% dplyr::mutate("Inf_1_kilo" = ifelse(raising_factor<0.001, TRUE, FALSE))

raisingfactorfromnumber_to_nominal_t_and_no%>% dplyr::filter(!is.na(Inf_1_kilo)) %>% dplyr::group_by(Inf_1_kilo) %>% dplyr::summarise(Nombre_de_strates = n())


```

En ne regardant que les strates ou il y a des données en deux unités, on constate qu'il y a environ 14000 strates pour lesquelles il faudrait un facteur de conversion de monis de 1 kilos pour ne pas dépasser les géoreferencées.

```{r}

qflextable2(raisingfactorfromnumber_to_nominal_t_and_no, captionn = "Nombre de strates avec un facteur de conversion de plus ou de moins de 1 kilos (strate engin/espèces/année/pavillon)")

```

Ca rerésente environ 12% du nombre de strates ayant des doubles unités pour un même engin/espèces/année/pavillon. On pourrait affiner par espèces mais cela paraît incohérent.

```{r}

try(convert_number_to_nominal_output_t_and_no <- convert_number_to_nominal(lvl0_conv_only_double_unit %>% dplyr::select(-c(measurement_type, measurement, measurement_status)), nominal%>% dplyr::select(-c(measurement_type, measurement, measurement_status)), raise_only_unmatched = FALSE, strata = c("species", "fishing_fleet", "year")))
raisingfactorfromnumber_to_nominal_t_and_no <- readRDS(here::here("data/raisingfactorfromnumber_to_nominal.rds")) %>% dplyr::mutate("Inf_1_kilo" = ifelse(raising_factor<0.001, TRUE, FALSE))
...
qflextable2(raisingfactorfromnumber_to_nominal_t_and_no, captionn = "Nombre de strates avec un facteur de conversion de plus ou de moins de 1 kilos (strate espèces/année/pavillon)")

```

Même en s'affranchissant de l'engin qui posait problème il y a toujours environ 11% de facteurs de conversion qui paraissent incohérents.

```{r}
try(convert_number_to_nominal_output_species_year <- convert_number_to_nominal(lvl0 %>% dplyr::select(-c(measurement_type, measurement, measurement_status)), nominal%>% dplyr::select(-c(measurement_type, measurement, measurement_status)), raise_only_unmatched = FALSE, strata = c("species", "year")))

raisingfactorfromnumber_to_nominal_t_and_no_only_species_year <- readRDS(here::here("data/raisingfactorfromnumber_to_nominal.rds")) %>% dplyr::mutate("Inf_1_kilo" = ifelse(raising_factor<0.001, TRUE, FALSE))

raisingfactorfromnumber_to_nominal_t_and_no_only_species_year%>% dplyr::filter(!is.na(Inf_1_kilo)) %>% dplyr::group_by(Inf_1_kilo) %>% dplyr::summarise(Nombre_de_strates = n())

```

```{r}

qflextable2(raisingfactorfromnumber_to_nominal_t_and_no_only_species_year, captionn = "Nombre de strates avec un facteur de conversion de plus ou de moins de 1 kilos (strate espèces/année)")
